...
static int handle_request(int fd) {
    ...
    if(client_ip){
        for(int i=0; i<wl_mem.ip_cnt; i++) {
            if(!strcmp(client_ip, wl_mem.ips[i])) { wl=true; break; }
        }
        if(!wl) {
            for(int i=0; i<wl_mem.cidr_cnt; i++) {
                if(ip_in_cidr_v4(client_ip, wl_mem.cidrs[i])) { wl=true; break; }
            }
        }
    }
    if(!wl && sender_dom) {
        for(int i=0; i<wl_mem.dom_cnt; i++) {
            if(!strcasecmp(sender_dom, wl_mem.doms[i])) { wl=true; break; }
        }
    }
    if(wl) {
        resp = "action=dunno\n\n";
    ...
    // 3. Passlist RAM (Buscamos la DUPLA para ver si ya es de confianza global)
    ...
    // 4. Greylist RAM (Buscamos el TRIPLET para el retry por usuario)
    ...
    while(*pg){
        if(!strcmp((*pg)->key, key_triplet)){
            if((time(NULL) - (*pg)->first_seen) >= cfg.delay_sec){
                // EXITO: Pasó la prueba para un usuario. Promocionamos la DUPLA a Passlist.
                ...
                sqlite3_bind_text(st,1,key_dupla,-1,SQLITE_TRANSIENT);
                sqlite3_bind_int64(st,2,time(NULL));
                sqlite3_bind_int(st,3,npe->hits);
    ...
    // 5. Nuevo (First Time) - Registramos el TRIPLET
    if(cfg.debug) log_msg("DEBUG", "FIRST TIME: %s", key_triplet);
    GreyEntry *nge = malloc(sizeof(GreyEntry));
    nge->key = strdup(key_triplet);
    nge->first_seen = time(NULL);
    nge->last_seen = time(NULL);
    nge->count = 1;
    nge->next = grey_table[h_t];
    grey_table[h_t] = nge;
...
static void config_defaults(){
    ...
    strcpy(cfg.listen_ip, "127.0.0.1");
